...
export const getStaticProps: GetStaticProps<DynamicProductPageProps> = async ({ params, locale, defaultLocale }) => {
  const slug = String(params?.stacionarus ?? "");
  const localeUsed = locale ?? defaultLocale;
  const qc = new QueryClient();
  try {
    await prefetchTowerBySlug(qc, slug);
  } catch (e) {
    console.error(e, "/stacionarus/[stacionarus] prefetch");
  }
...
export function StacionarusProductPage({ slug }: InferGetStaticPropsType<typeof getStaticProps>) {
...
export default StacionarusProductPage;
