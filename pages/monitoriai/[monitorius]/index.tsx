...
export const getStaticProps: GetStaticProps<DynamicProductPageProps> = async ({ params, locale, defaultLocale }) => {
  const slug = String(params?.monitorius ?? "");
  const localeUsed = locale ?? defaultLocale;
  const qc = new QueryClient();
  try {
    await prefetchMonitorBySlug(qc, slug);
  } catch (e) {
    console.error(e, "/monitoriai/[monitorius] prefetch");
  }
...
export function MonitoriusProductPage({ slug }: InferGetStaticPropsType<typeof getStaticProps>) {
...
export default MonitoriusProductPage;
